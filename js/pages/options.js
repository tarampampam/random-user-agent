...
(function() {
  ...
  var gui = {
    ...
    live: {
      ...
      checkbox: function(api_get_method, api_set_method, checkbox_id) {
        chrome.runtime.sendMessage({action: api_get_method}, function(is_enabled) {
          typeof is_enabled === 'boolean' ? UI.getElementById(checkbox_id, function($el) {
            UI.isCheckbox($el) && ($el.checked = is_enabled, UI.addEvent($el, 'change', function() {
              chrome.runtime.sendMessage({
                action: api_set_method, data: {enabled: $el.checked}
              });
            }));
   ...
   * Renew interval
   */
  chrome.runtime.sendMessage({action: 'settings.getRenewInterval'}, function(interval) {
    typeof interval === 'number' ? UI.getElementById('auto_renew_interval', function($el) {
      if (UI.isInputNumber($el) || UI.isInputText($el)) {
        ...
        UI.addEvent($el, 'keyup', function() {
          ...
          chrome.runtime.sendMessage({
            action: 'settings.setRenewInterval', data: {interval: $el.value}
          });
        });
      }
   ...
   * Custom user-agent
   */
  chrome.runtime.sendMessage({action: 'settings.getCustomUseragent'}, function(useragent) {
    UI.getElementById('custom_useragent', function($el) {
      if (UI.isInputText($el)) {
        ...
        UI.addEvent($el, 'keyup', function() {
          ...
          chrome.runtime.sendMessage({
            action: 'settings.setCustomUseragent', data: {useragent: $el.value}
          });
        });
      }
    });
   ...
   */
  chrome.runtime.sendMessage({action: 'generator.getTypes'}, function(types) {
              ...
              if (generator_browsers.hasOwnProperty(major)) {
                if (generator_browsers[major].hasOwnProperty(minor)) {
                  ...
                  if (browser_generator.hasOwnProperty('name')) {
                    ...
                    generators.push({name: name, type: type});
                  }
                }
              }
            }
          ...
          UI.forEachCssClass(checkbox_class, function($el) {
            UI.addEvent($el, 'change', function() {
              ...
              if (typeof all_checkboxes === 'object' && all_checkboxes.length > 0) {
                ...
                chrome.runtime.sendMessage({
                  action: 'generator.setTypes', data: {types: new_types}
                }, function(result) {
                  result !== true && console.error('API method "generator.setTypes" failed');
                });
              }
              chrome.runtime.sendMessage({action: 'useragent.renew'}, function(){});
            });
          });
        }
      });
   ...
   * Exceptions list
   */
  chrome.runtime.sendMessage({action: 'exceptions.get'}, function(exceptions) {
      ...
      UI.getElementById('exceptions_patterns', function($el) {
        if (UI.isTextarea($el)) {
          ...
          UI.addEvent($el, 'keyup', function() {
            ...
            chrome.runtime.sendMessage({
              action: 'exceptions.set', data: {exceptions: new_exceptions}
            }, function(result) {
